...
class ProspectorHyperParams(ProspectorParams):
    ...
    def __init__(self,*args,sfh_psd=None,psd_freqs=None,**kwargs):
        """
        sfh_psd = some arbitrary psd as a function of the psd_freqs that governs the SFH. 
        """
        super().__init__(*args, **kwargs)

        self.sfh_psd = sfh_psd
        self.sfh_psd_freqs = psd_freqs


    # --- since we aren't actually fitting for the PSD (since it is held fixed), we don't need the additional fitting functions!
    ...
    #     # psd_params = np.zeros(len(self.hyperparams))
